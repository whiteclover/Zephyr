...
from tornado.util import u, unicode_type, basestring_type

string_types = (basestring_type, unicode_type)
...
class AtomFeed(object):
    ...
    def generate(self):
        ...
        if self.logo:
            yield u('  <logo>%s</logo>\n') % escape(self.logo)
        if self.rights:
            yield u('  ') + _make_text_block('rights', self.rights,
                                          self.rights_type)
...
class FeedEntry(object):
    ...
    def generate(self):
        ...
        base = ''
        if self.xml_base:
            base = u(' xml:base="%s"') % escape(self.xml_base)
        yield u('<entry%s>\n') % base
        yield u('  ') + _make_text_block('title', self.title, self.title_type)
        yield u('  <id>%s</id>\n') % escape(self.id)
        yield u('  <updated>%s</updated>\n') % format_iso8601(self.updated)
